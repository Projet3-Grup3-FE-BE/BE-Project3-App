...
func New(e *echo.Echo, srv domain.ServiceInterface) {
	handler := orderHandler{srv: srv}
	e.POST("/orders", handler.AddOrder(), middleware.JWT([]byte(key))) //jangan lupa dikasih jwt soalnya habis di hapus buat coba
	// e.GET("/orders", handler.GetAllOrder())
...
// func (us *orderHandler) GetAllOrder() echo.HandlerFunc {
...
// 		return c.JSON(http.StatusOK, SuccessResponse("Success get orders.", ToResponse(res, "orders")))
// 	}
// }

func (us *orderHandler) GetOrder() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		return c.JSON(http.StatusOK, SuccessResponse("Success get order.", ToResponse(res, "order")))
	}
}
...
func (us *orderHandler) AddOrder() echo.HandlerFunc {
	return func(c echo.Context) error {
		// Check authorized request atau tidak dgn token
		...
		var input OrderInsertRequestFormat
		...
		if isSuccess {
			...
			log.Println("\n\n\n input order handler : ", input, "\n\n\n")
			id := jwt.ExtractIdToken(c)
			cnv := ToDomain(input)
			cnv.IdUserBuyer = id
			log.Println("\n\n\n input order handler cnv : ", cnv, "\n\n\n")

			res, err := us.srv.Insert(cnv, c)
			if err != nil {
				return c.JSON(http.StatusInternalServerError, FailResponse(err.Error()))
			}
			return c.JSON(http.StatusCreated, SuccessResponse("Success add order.", ToResponse(res, "order")))
...
func (us *orderHandler) DeleteOrder() echo.HandlerFunc {
	return func(c echo.Context) error {
		// Check authorized request atau tidak dgn token
		err := us.srv.IsAuthorized(c)
		if err != nil {
		...
		if paramID == "" {
			...
			if err != nil {
				...
				if err2 != nil {
					...
					return c.JSON(http.StatusCreated, SuccessDeleteResponse("Success delete order."))
				}
			}
		}
